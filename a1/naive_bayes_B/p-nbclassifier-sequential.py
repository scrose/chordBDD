...
def main ():    
    ...
    df, prior, condprob = nb_multi_train(df, train_label, vocab)
        
    print("Applying model to training data...")
    prediction = nb_multi_apply(df, prior, condprob)
...
def nb_multi_train (df, train_label, vocab_df):
    ...
    for bad in bad_list:
        for j in range(1,21):
            Pr_dict[j][bad] = a/(pb_j['count'].sum()[j] + v_count)
