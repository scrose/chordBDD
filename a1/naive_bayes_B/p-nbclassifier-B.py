...
=========================================
Implementation B: Multithreading
...
# Main: Read & classify documents; Output file: results.txt
def main ():    
    ts = time()
    ...
    # Extract vocabulary
    vocab = extract_vocab('data/20news-bydate/vocabulary.txt')
            
    # Build training model  
    print("Building training model...")
    df, pi, Pr_dict = nb_multi_train(df, train_label, vocab)
    ...
    for bad in bad_list:
        ...
        for j in range(1, 21):
            threads.append(Thread(
                    target=calc_pi, args=(results, j, 21, a, pb_j, v_count)))
            threads[j-1].start()
        
        for x, t in enumerate(threads):
            t.join()
            Pr_dict[x+1][bad] = results[x+1]
...
def nb_multi_apply(df, pi, Pr_dict):
    ...
    for docIdx in range(1, len(new_dict)+1):
        score_dict = {}
        # Create a task queue for the probability rows
        # Creating a probability row for each class
        for classIdx in range(1,21):
            threads = []
            for j in range(1, 21):
                threads.append(Thread(
                        target=calc_score, 
                        args=(score_dict, new_dict, Pr_dict, docIdx, classIdx, pi)))
                threads[j-1].start()
            for x, t in enumerate(threads):
                t.join()
...
# =======================================
def calc_pi(results, j, cls, a, pb_j, v_count):
    results[j] = (a/(pb_j['count'].sum()[j] + v_count))
    return results

     
# =======================================    
# Bottleneck 2: Score calculation
def calc_score(results, new_dict, Pr_dict, docIdx, classIdx, pi):
    results[classIdx] = 1
    for wordIdx in new_dict[docIdx]:
        try:
            probability = Pr_dict[wordIdx][classIdx]        
            power = new_dict[docIdx][wordIdx]               
            results[classIdx] += power*np.log(probability)                     
        except:
            #Missing V will have 0*log(a/16689) = 0
            results[classIdx] += 0
    #Multiply final with pi         
    results[classIdx] +=  np.log(pi[classIdx])
    return True
